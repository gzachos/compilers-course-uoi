...
\newcommand{\hmwkDueDate}{Monday,\ May\ 24,\ 2017} % Due date
...
\date{March 20, 2017} % Insert date here if you want it to appear below your name
...
can be referenced using a label (non-negative integer).
...
\begin{itemize}
 ...
 \item \code{backpatch(list, res)}: Replaces the \code{res} attribute\footnote{3rd operand.} of all the quadruples referenced by
                                    the labels in \code{list}.
...
\textbf{Notes}:
\begin{itemize}
 \item The generated C code is ready to compile using a C compiler.
 \item The transformation to ANSI C code takes place only when nested function definitions do not exist
       in user program, as it is not supported by the language specifications. On a later version of CSC,
       we plan to support nested functions using a GNU C extension provided by the GNU C
       Compiler\footnote{https://www.gnu.org/software/gnu-c-manual/gnu-c-manual.html\#Nested-Functions}.
\end{itemize}
